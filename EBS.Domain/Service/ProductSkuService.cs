...
namespace EBS.Domain.Service
{
    ...
    {
        IDBContext _db;
        public ProductSkuService(IDBContext dbContext)
        {
            this._db = dbContext;
        }

        public void Create(ProductSku model)
        {
            if (_db.Table.Exists<ProductSku>(n => n.Name == model.Name))
            {
        ...
        public void Update(ProductSku model)
        {
            if (_db.Table.Exists<ProductSku>(n => n.Name == model.Name && n.Id != model.Id))
            {
                throw new Exception("名称重复!");
            }
            _db.Update(model);
        ...
        public void PublishToggle(string ids,bool isPublish)
        {
            ...
            var arrIds = ids.Split(',').ToIntArray();
            var products= _db.Table.Find<ProductSku>(arrIds);
            foreach (var item in products)
            {
                item.IsPublish = isPublish;
            }
            _db.Update<ProductSku>(products.ToArray());
        }
    }
}
